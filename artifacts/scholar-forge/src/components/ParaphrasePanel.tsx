...
import { Sparkles, RefreshCw, BookmarkPlus, Check } from "lucide-react";
...
export function ParaphrasePanel({
  ...
  const { addParaphrase, isInCollection } = useCollection();
  ...
  }, [text, intensity.value, config?.discipline, authorsStr, paper]);

  const handleSave = () => {
    const text = editedParaphrase.trim();
    if (!text) return;
    const citationStr = result?.citationInline ?? `(${authorsStr}, ${paper.year ?? "n.d."})`;
    const { added, total } = addParaphrase(text, citationStr, paper);
    if (added) {
      setSaved(true);
